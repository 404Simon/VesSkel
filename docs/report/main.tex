...
\begin{abstract}
  ...
  an MIT-licensed Python package that unifies skeletonization,
  ...
  than existing alternatives while producing bit-identical output, a
  novel junction-cleanup step that removes thinning-induced triangle
  artifacts, and dual napari and CLI interfaces with fully
  serializable JSON configuration for reproducible batch processing.
  On the High-Resolution Fundus dataset, an SVM trained on VesSkel
  features achieves 91.1\% accuracy in discriminating healthy,
  diabetic retinopathy, and glaucoma subjects, confirming that its
  features encode biologically relevant vascular phenotypes.
\end{abstract}
