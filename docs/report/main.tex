...
\begin{abstract}
  ...
  provides clinically relevant biomarkers for diseases including
  diabetic retinopathy and glaucoma.   A common
  approach~\cite{skel_survey} extracts these biomarkers by
  skeletonizing binary vessel segmentations and deriving graph-based
  features from the resulting centerline representation. However,
  this workflow requires assembling heterogeneous tools that differ
  in license, supported dimensionality, interface, and feature scope
  --- a fragmented process that introduces reproducibility risks and
  undermines quantitative rigor. To address this, we present VesSkel,
  an open-source-licensed Python package that unifies skeletonization,
  artifact removal, and phenotype feature extraction in a single
  end-to-end pipeline.   VesSkel provides a Numba-parallelized Lee94
  thinning implementation that is 5--9$\times$ faster in 2D
  (0.084\,s/image, $3504\times2336$\,px) than existing alternatives
  while producing bit-identical output, a novel junction-cleanup step
  that removes thinning-induced triangle artifacts, and dual napari
  and CLI interfaces with fully serializable JSON configuration for
  reproducible batch processing.  From each image, VesSkel extracts
  29 per-image features spanning topology, morphology, and density.
  On the High-Resolution Fundus dataset, an SVM trained on
  ANOVA-selected VesSkel features achieves 91.1\% accuracy in
  discriminating healthy, diabetic retinopathy, and glaucoma subjects,
  confirming that its features encode biologically relevant vascular
  phenotypes.
\end{abstract}
...
\Cref{fig:svm-k-sweep}) confirms that the top ten features provide
...
speedups. The 3D Lee94 implementation, while competitive, leaves room
for improvement: a wavefront-based parallel deletion scheme could
reduce the sequential bottleneck in the removal phase.
...
Repository~\cite{VesSAP} ($600\times304\times325$~voxels). The raw
volume was preprocessed by applying a median filter with radii $(x,
y, z) = (1, 1, 0)$, followed by Li thresholding to obtain a binary
vessel mask. We also use the scikit-image brain volume~\cite{skimage}.
...
CC-BY 4.0 license. The VesSAP test volume~\cite{VesSAP} was
downloaded from
\url{https://github.com/vessap/vessap/blob/master/data/1.nii.gz}. The
...
CSV files and the binarized VesSAP 3D Test-Volume) are included in
the VesSkel repository under \texttt{tests/}.
...
\input{../../tables/top_anova_feature_means.tex}
