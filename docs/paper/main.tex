...
\begin{abstract}
  ...
  diabetic retinopathy and glaucoma. A common approach extracts these
  biomarkers by skeletonizing binary vessel segmentations and deriving
  graph-based features from the resulting centerline representation.
  However, this workflow requires assembling heterogeneous tools that
  differ in license, supported dimensionality, interface, and feature
  scope --- a fragmented process that introduces reproducibility risks
  and undermines quantitative rigor. To address this, we present
  VesSkel, an MIT-licensed Python package that unifies skeletonization,
  ...
  thinning implementation that is up to 5--9$\times$ faster in 2D
  than existing alternatives while producing bit-identical output, a novel
  junction-cleanup step that removes thinning-induced triangle
  artifacts, and dual napari and CLI interfaces with fully serializable
  JSON configuration for reproducible batch processing. On the
  High-Resolution Fundus dataset, an SVM trained on VesSkel features
  achieves 91.1\% accuracy in discriminating healthy, diabetic
  retinopathy, and glaucoma subjects, confirming that its features
  encode biologically relevant vascular phenotypes.
\end{abstract}
...
through distinct pathological mechanisms. The full set of group-wise
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.7\linewidth]{figures/final_model_comparison.pdf}
