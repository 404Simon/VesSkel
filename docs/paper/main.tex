...
% This template provides an example of a standard structure that is
% used for many scientific articles. Depending on the content you
% present in your thesis, report, or student paper, it can make sense
% to use this structure or to deviate from it. The only exceptions are
% the sections \Cref{sec:code,sec:data}. These sections are mandatory
% and must be filled according to the provided descriptions. Moreover,
% you must fill-in the AI usage declaration in
% \Cref{sec:ai-declaration} and date and sign the declaration of
% academic integrity in \Cref{sec:declaration}.
%
% In the introduction, you provide the motivation of your work, pose
% your research question, and describe the (medical and technical)
% background as well as the related work (if these parts are rather
%   extensive they can also be put in dedicated ``Background'' and/or
% ``Related work'' sections). Make sure that the contribution of
% \emph{your} work becomes clear in the introduction: What does it add
% to the existing body of literature/knowledge? Why is it new and/or
% interesting? Conclude the introduction by delineating the structure
% of your thesis, i.\,e., by introducing the structure of the remaining
% sections. The introduction should be internally unstructured (no
% subsections inside the introduction).

- hrf als motivation: diabetes \& glaucoma diagnosis
- limitations of existing vessel analysis tools
- überleitung zu need for FOSS package mit nicer ux (napari,
reproducible config, cli, fast)
- ?? Related work: vessel segmentation, skeletonization (Lee94,
skimage), phenotype analysis
- General Tool Comparison table (nicht features)
- evtl. lee und skan als basis erwähnen
- vorstellung vesskel
- (1) Numba-parallelized 2D/3D Lee94 implementation, (2) integrated
feature extraction pipeline, (3) napari plugin + CLI, (4) application
to HRF dataset with phenotype analysis
- struktur des papers
...
vorstellung des high-level workflow:

1. binary mask
2. (optional) preprocessing (closing, hole-filling)
3. Lee thinning
4. (optional) junction cleanup
5. graph construction
6. feature extraction

pipeline verfügbar als napari plugin (interactive, parameter tuning,
config save/load) und CLI für batching

- Figure: pipeline diagram + evtl. example input/skeleton/branches/features

\subsection{Skeletonization Performance}\label{sec:results:performance}

zahlen und fakten

- 2D und 3D validation on HRF dataset und brain; skimage als base und
regression tests against pre-computed baselines stellen korrektheit sicher
- numba-parallelized: x speedup vs. pure-Python reference (initiale
naive iml hat ja iwie 20min für HRF gebraucht)
- single vs multi thread -> scalability within image aber auch über
batch von images demonstrieren?
...
- nur kurz wichtigste punkte an denen ich ansetze erklären und lee
paper referenzieren

- Topology-preserving: 6-directional sub-iteration, border-pixel
removal preserving
- (1) endpoints
- (2) Euler characteristic
- (3) simple-point condition
...
- in 2d euler fällt in 2D weg (proof übernehmen?)
- usw.
...
- closing und hole filling der maske erwähnen
- Triangle/diamond artifacts from skeletonization detected via
cycle-basis on skan graph: cycles with perimeter < 2.5x local vessel
diameter are collapsed to a centroid pixel.
...
- HRF dataset: 45 fundus images (15 healthy, 15 DR, 15 glaucoma) with
manual vessel segmentation masks
- 3D: brain und vessap volume erwähnen
- KEINE medizinischen details -> nur kurzer satz zu krankheiten und
dass die blutgefäße sich verändern
- evtl. nach 5. schieben

\subsection{Experimental Setup}\label{sec:methods:setup}

\paragraph{Runtime Tests}\label{sec:methods:setup:runtime}

- Hardware, feature configuration (junction cleanup on, radius on,
fractal dimension on, hole filling with max\_hole\_size=200).
...
Gemini, Chatgpt, Copilot, Deepseek, Opencode
