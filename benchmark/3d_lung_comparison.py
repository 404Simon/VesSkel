"""3D Lung Benchmark: vesskel vs skimage vs VesselVio on VESSEL12 scans.
...
from vesskel.pipeline import preprocess_binary
...
def print_row(name, lee_t, sk_lee_t, vv_t, speedup_sk=None, speedup_vv=None):
    if speedup_sk is None and speedup_vv is None:
        ...
        print(
            ...
            f"{str(speedup_sk):<12} {str(speedup_vv):<12}"
        )


def main():
    print("=" * 95)
    print("  VESSEL12 Lung Benchmark: vesskel vs skimage vs VesselVio")
    ...
    print("=" * 95)
    print_row("Stat", "vesskel(s)", "sk lee(s)", "vv lee(s)", "Spdup sk", "Spdup vv")
    print("-" * 85)
    stats = [
